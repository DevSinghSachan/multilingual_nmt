...
NAME="run_en_ja_wpm_large"
OUT="temp/$NAME"

DATA=${HOME}"/enja_datasets"
TRAIN_SRC=$DATA/train.en
TRAIN_TGT=$DATA/train.ja
TEST_SRC=$DATA/test.en
TEST_TGT=$DATA/test.ja
VALID_SRC=$DATA/dev.en
VALID_TGT=$DATA/dev.ja
...
python ${TF}/preprocess.py -i ${OUT}/data \
      ...
      -s-test test.src \
      -t-test test.tgt \
      --save_data processed --max_seq_len ${MAX_SEQ_LEN}
...
CMD="python ${TF}/train.py -i ${OUT}/data --data processed \
         ...
         --metric bleu --wbatchsize 12000 --model Transformer \
         --grad_accumulator_count 2 --warmup_steps 8000 --eval_steps 5000 --fp16 --dynamic_loss_scale \
         --n_units 1024 --n_hidden 4096 --multi_gpu 0 1 2 3"
